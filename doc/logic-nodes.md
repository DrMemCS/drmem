...
This document shows several examples of how internal control might be described in the DrMem config file. This is a proposed feature and hasn't yet been implemented. The components used in expressions, in particular, are not documented at all and may look completely different when actually supported.

It should be noted that, in real config files, all `[[driver]]` blocks occur first. `[[logic]]` sections must be last because they use devices which must be already defined.

Logic sections have 5 recognized keys:

| key | description |
|-----|-------------|
| `defs` | A map containing expressions. Expressions in `exprs` can refer to these entries to simplify or share definitions. Expressions can only use devices found in `inputs`. |
...
| `name` | A name for the block. This name is only used to mark log messages. |
